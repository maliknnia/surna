...
import { mergeProfileEvents, type DemoProfileEvent } from "@/lib/demoProfileMedia";
...
export function ProfileEventsPanel({ userId, isOwnProfile }: ProfileEventsPanelProps) {
  ...
  const events = mergeProfileEvents(apiEvents, isOwnProfile) as (EventRow | DemoProfileEvent)[];

  if (isLoading) {
    return (
      <div className="space-y-3">
        {Array.from({ length: 2 }).map((_, i) => (
          <div key={i} className="h-28 rounded-2xl animate-pulse" style={{ background: "var(--surna-elevated)" }} />
        ))}
      </div>
    );
  }
    ...
    return (
      <ProfileSectionCard title="Events">
        <div className="text-center py-8">
          <Calendar className="w-12 h-12 mx-auto mb-3 opacity-30" style={{ color: "var(--surna-text)" }} />
  ...
  return (
    <div className="space-y-3">
      {events.map((event) => {
        const loc = locationLabel(event.location as EventRow["location"]);
        return (
